...
def already_installed_client_server(traffic):
    ...
    for other_traffic in traffic_list:
        if (
            ...
            traffic.set_src_id(other_traffic.src_id)
            traffic.set_dst_id(other_traffic.dst_id)
            traffic.set_sfcr_id(other_traffic.sfcr_id)        
...
def install_client_and_server(traffic):
    ...
    print("Succes to install server and client for traffic generation")
    ...
    print("Success to create grafana dashboard")
...
def iperf3_activate(traffic):
    ...
    #while(not get_sfc_by_name(get_sfcr_by_id(traffic.sfcr_id).name)):
    #    print("Waiting for setup SFC")
    #    time.sleep(10)
    ...
    #Iperf3 on server
    inner_command = "iperf3 -s -D -1 -p "+str(udp_port)
    ...
    for i in range(0, limit):
        ...
        if limit == 9:
            print("Iperf3 server subprocess failed")
            return
   
    print("Iperf3 is activated on {}".format(get_ip_from_id(traffic.dst_id)))

    udp_port += 1

    #check used pids & ports
    inner_command = "lsof -i -P -n | grep iperf3 | awk '$9 !~ /[>]/ {print $2, $9}' | tr -d ':*'"
    inner_command = (ssh_command + inner_command) % iperf3_server
    command = (ssh_command + inner_command) % traffic_controller     
    response2 = subprocess.check_output(command, shell=True).strip().decode("utf-8")

    response2 = response2.split('\n')
    pid, port = find_pid_port(response2)


    #fix for response.slplit()

    traffic.set_process_id(pid)
    traffic.set_port(port)

    #Iperf3 on client
    inner_command = "iperf3 -c %s -b %sM -u -t %s --length 1200 -p %s -logfile test" % (get_ip_from_id(traffic.dst_id), traffic.bandwidth, traffic.duration, traffic.port)
    inner_command = (ssh_command + inner_command) % iperf3_client
    command = (ssh_command + inner_command) % traffic_controller
    subprocess.Popen(command, shell=True)
    
    print("Iperf3 is activated on {}".format(get_ip_from_id(traffic.src_id)))

    print("success to set iperf")
    return response
